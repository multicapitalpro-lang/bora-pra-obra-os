...
if($id){$data[]=$id;$sql='UPDATE capitulos SET numero=?,titulo=?,data_gravacao=?,qtd_arquivos=?,temporada_id=?,etapa=?,status=?,drive_url=?,youtube_url=?,observacoes=? WHERE id=?';}else{$sql='INSERT INTO capitulos(numero,titulo,data_gravacao,qtd_arquivos,temporada_id,etapa,status,drive_url,youtube_url,observacoes) VALUES(?,?,?,?,?,?,?,?,?,?)';}
